...
onready var dialog: Array = get_parent().get_parent().dialog
var page: int = 0
var away : bool = true
...
func _process(delta):
	var pos : Vector2 = get_parent().get_parent().position;
	
	if pos.distance_squared_to(Player.position) < get_parent().get_parent().squareRange:
